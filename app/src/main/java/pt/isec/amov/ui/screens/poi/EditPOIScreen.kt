...
@Composable
fun EditPOIScreen(navController: NavHostController, vm: ActionsViewModel, poi: PointOfInterest) {
    var locationName by remember { mutableStateOf(poi.name) }
    var locationDescription by remember { mutableStateOf(poi.description) }
    var expanded by remember { mutableStateOf(false) }
    var selectedCategory by remember { mutableStateOf(poi.category) }
    var showLatLonDialog by remember { mutableStateOf(false) }
    var latitude by remember { mutableStateOf(poi.latitude) }
    var longitude by remember { mutableStateOf(poi.longitude) }
    var updatedPOI = remember { mutableStateOf(poi.copy()) }
    ...
    Column(
        ...
        if (poi.photoUrl != "") {
            ...
            val storageRef: StorageReference? = if (poi.photoUrl!!.isNotBlank()) {
                storage.reference.child(poi.photoUrl!!)
            } else {
                null
            }
        ...
        NormalBtn(
            onClick =
            {
                if (locationName.isNotBlank() && locationDescription.isNotBlank()
                    ...
                    updatedPOI.value.name = locationName
                    if(vm.imagePath.value != null) {
                        updatedPOI.value.photoUrl = "images/"+vm.imagePath.value
                        StoreUtil.deleteImagesFromStorage(poi.photoUrl!!)
                    }
                    updatedPOI.value.description = locationDescription
                    updatedPOI.value.category = selectedCategory
